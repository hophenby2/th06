...
--				local angle=105-i*num
				...
				if self.sp[i] and self.sp[i][3]>0.5 then
					...
					if target then
						...
						if target.class.base.take_damage then target.class.base.take_damage(target,0.25) end
...
function marisa_player:spell()
	...
	if self.slow==0 then
		...
		self.nextshoot=300
		self.protect=360
		New(marisa_spark,self.x,self.y,90,30,240,30,self)
--		self.hspeed=1
--		self.lspeed=1
		New(tasker,function()
			for i=1,27 do
				New(marisa_spark_wave,self.x,self.y-16,90,12,0.9,self)
				task.Wait(10)
			end
			New(bullet_killer,self.x,self.y)
			PlaySound('slash',1.0)
...
function marisa_sp_ef:init(rot,player)
...
function marisa_spark_wave:init(x,y,rot,v,dmg,p)
	self.player=p
	self.x=x
	self.y=y
	self.rot=rot
