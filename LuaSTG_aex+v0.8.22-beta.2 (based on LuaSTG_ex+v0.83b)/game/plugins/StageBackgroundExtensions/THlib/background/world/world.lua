...
function world_background:frame()
	...
	if self.timer>600 and self.timer<2600 then
		Set3D('at',0.0+0.5*sin((self.timer-600)*0.18),0.7+0.6*sin((self.timer-600)*0.18),-3.6+0.6*sin((self.timer-600)*0.18))
	end
...
function world_background:render()
	...
	for i=-6,40 do
		for j=-15,15 do
--		Render4V('blue_line',-1,-1+j,1-z+i, 1,-1+j,1-z+i, 1,-1+j,-1-z+i,-1,-1+j,-1-z+i) ����
