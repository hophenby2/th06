...
function gzz_stage04bg_background:init()
	...
	--resource
	LoadImageFromFile('gzz_stage03a','THlib/background/gzz_stage04bg/gzz_stage04a.png')
	LoadTexture      ('gzz_stage03b','THlib/background/gzz_stage04bg/gzz_stage04b.png')
	LoadImageGroup('china','gzz_stage03b',1,0,255,16,1,16)
	LoadImageFromFile('gzz_stage03g','THlib/background/gzz_stage04bg/gzz_stage04g.png')
	LoadImageFromFile('gzz_stage03c','THlib/background/gzz_stage04bg/gzz_stage04c.png')
	LoadTexture      ('gzz_stage03f','THlib/background/gzz_stage04bg/gzz_stage04f.png')
	LoadImageGroup('gzz_3_light','gzz_stage03f',0,0,32,32,4,1)
	LoadTexture      ('gzz_stage03e','THlib/background/gzz_stage04bg/gzz_stage04e.png')
...
function gzz_stage04bg_background:render()
	...
	for j=-4,5 do
		...
		gzz_stage04bg_background.renderfloor(dz)
	end
	for j=-4,5 do
		local dz=j*2-math.mod(self.z,2)
		local t= self.timer
		gzz_stage04bg_background.renderlightR(4.135,1,dz,1.5,t)
	end
	for j=-4,5 do
		local dz=j*2-math.mod(self.z,2)
		gzz_stage04bg_background.renderwall(dz)

	end
	for j=-4,5 do
		local dz=j*4-math.mod(self.z,4)
		local t= self.timer
		gzz_stage04bg_background.renderlightL(3.625,1,dz,1.5,t)
	end
	for j=-4,5 do
		local dz=j*2-math.mod(self.z,2)
		gzz_stage04bg_background.renderstone(dz)

	end
	local offset=0.2
	for j=-4,5 do
		local dz=j*2-math.mod(self.z,2)
		gzz_stage04bg_background.renderchina(2.65,5.0,2+2+offset,2.5+2.5+offset,2,dz)
	end
	end
...
gzz_stage03bg=gzz_stage04bg_background
