...
|---|---|---|---|---|---|
| B01 | `normalize_target_args_for_op_key` | TH13+ movement float index normalization and TH12 tween-mode clamping | `target/arg_adapter` op layouts / target argument schema | todo | Backend should not know per-op argument semantic indices. |
| B02 | `UNSAFE_TARGET_OPCODES` | thecl/thtk format table hazards | `dialects/reference.py` or `tool_abi.py` | keep-backend | Tool ABI metadata, not IR semantic; can move out of backend later. |
| B03 | removed `compile_th08_vm_arithmetic` | TH06-08 expression ops lowered to stack VM instruction sequences | `canonical/op_ir.op_lowering_policy` / `LegacyVmOp` sequence templates | done | Verified simple stack ops use policy templates; geometry helpers stay explicit unsupported until a typed temporary allocator exists. |
| B04 | `compile_th08_movement_alias` | TH06-08 `move_dir*` aliases to velocity ops | `canonical/op_ir.op_lowering_policy` alias templates | done | Function deleted; handled by IR op policy. |
| B05 | `compile_th08_conditional_jump` | TH06-08 comparison+jump/loop lowered to stack compare + jump | `canonical/op_ir.op_lowering_policy` control-flow templates | done | Function deleted; comparison and loop policies emit target compare/jump sequences. |
| B06 | `compile_th08_anm_alias` | TH06-08 `anm.set`/slot aliases | `canonical/op_ir.op_lowering_policy` sequence template | done | Function deleted; handled by IR op policy. |
...
| B09 | `compile_ir_op_event` pre-fallback checks | Disabled async, float-time, nop, transform unsafe handling | `canonical/op_ir.lowering_policy` / transform IR | partial | Disabled async and float-time are policies; nop and TH13+ transform-to-TH12 still backend checks. |
...
| B12 | `remap_named_args` ANM role/script remaps | ANM bank/script semantic remapping | typed `AnmResourceRef` + ordered bank-flow analysis | partial | Canonical cross-game resource ops are now gated as unsupported; the legacy path still uses role heuristics and numeric remaps. |
...
| B19 | `clamp_old_shape` | Old-target bullet shape bounds | target bullet-shape catalog metadata | partial | Canonical lowering is source-first and catalog-driven; the legacy `compile_th08_macro` path still clamps numeric shapes. |
...
| B21 | backend argument emission | Per-game variable rewrites | `VariableDialect` / `RoutineDialect` projection before backend selection | done | Canonical planner, preemptive policies, and argument adapters share the typed projector; no pairwise variable regex table remains. |
