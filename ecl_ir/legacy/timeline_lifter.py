...
def lift_timelines(program: Program) -> list[TimelineOp]:
    ...
    for func in program.functions:
        ...
        timeline.fields = {
            ...
            "statements": [statement_to_event(stmt, program.game, func.name, program.source) for stmt in func.statements],
...
def statement_to_event(stmt: Statement, game: str, routine: str = "", source: str = "") -> dict[str, object]:
    event = {
        "kind": stmt.kind,
        "line": stmt.line_no,
        "text": stmt.text,
    ...
    if stmt.kind == "instruction" and "opcode" in stmt.attrs:
        event["semantic_op"] = semantic_operation(
            game,
            int(stmt.attrs.get("opcode", -1)),
            [str(arg) for arg in stmt.attrs.get("args", [])],
            stmt.line_no,
            stmt.difficulty,
            routine=routine,
            source=source,
