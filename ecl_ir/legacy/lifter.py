...
from ..analysis.spread_ir import double_flower_lowering_for_th12
from ..analysis.transform_ir import annotate_th12_to_th13plus_transforms
from ..canonical.op_ir import semantic_operation
from ..dialects.semantics import bullet_shape_semantic, spread_semantic
from ..source.model import Function, Instruction, Program
...
def append_emitter_op(emitter: BulletEmitter, ins: Instruction) -> None:
    emitter.semantics.setdefault("operations", []).append(
        semantic_operation(
            emitter.game,
            ins.opcode,
            ins.args,
            ins.line_no,
            ins.difficulty,
            routine=emitter.function,
            source=str(getattr(emitter, "source", "")),
            raw=ins.raw,
        ).to_dict()
    )
