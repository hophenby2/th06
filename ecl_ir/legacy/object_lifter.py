...
from ..canonical.op_ir import op_key_for_opcode, semantic_operation
from ..dialects.semantics import bullet_transform_mode_semantic, remap_bullet_transform_mode, remap_unit_flag_mask, unit_flag_semantics
...
def make_obj(cls, program: Program, func: Function, ins: Instruction, family: str, object_id: str = "0") -> IRObject:
    ...
    obj.raw.append(ins)
    obj.fields.setdefault("operations", []).append(
        semantic_operation(
            program.game,
            ins.opcode,
            ins.args,
...
def lift_enemy_visuals(program: Program, func: Function) -> list[IRObject]:
    ...
    for index, ins in enumerate(body):
        ...
        obj.raw.append(ins)
        if obj.fields.get("operations"):
            obj.fields["operations"] = obj.fields["operations"][-1:]
        append_ir_op(obj, program.game, ins)
