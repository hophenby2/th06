...
THTK_THECL06 = ROOT.parent / "thtk" / "thecl" / "thecl06.c"
...
def _parse_thtk_format_array(
    ...
    for opcode, signature in re.findall(r"\{\s*(\d+)\s*,\s*\"([^\"]*)\"\s*\}", match.group(1)):
        infos[int(opcode)] = OpcodeInfo(
            game=game,
            opcode=int(opcode),
            signature=signature,
            source=f"{source_path}:{array_name}",
        )
    return infos


def _parse_thtk_formats() -> dict[str, dict[int, OpcodeInfo]]:
    arrays: dict[str, tuple[Path, list[str]]] = {
        "th06": (THTK_THECL06, ["th06_fmts"]),
    ...
    for game, (source_path, chain) in arrays.items():
        ...
        for array in chain:
            for opcode, info in _parse_thtk_format_array(source_path, array, game).items():
