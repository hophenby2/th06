...
def compatibility_backend_emitter(
    ...
    projected, issues = project_semantic_operation(node, target_game)
...
class LoweringPlanner:
    ...
    def _plan_semantic_operation(self, node: SemanticOperation, routine: str) -> CapabilityDecision:
        ...
        if node.selected_values:
            return self._unsupported(
                node,
                routine,
                code="value_selection.unsupported",
                message=(
                    "target lowering for runtime-selected source values is not implemented; "
    ...
    def _plan_raw_instruction(self, node: RawInstructionOp, routine: str) -> CapabilityDecision:
        ...
        if node.selected_values and not same_game:
            return self._unsupported(
                ...
                code="value_selection.unsupported",
                message=(
                    "raw instruction depends on runtime-selected source values that cannot "
                    "be passed through independently"
                ),
            )
        if not allowed:
            return self._unsupported(
        ...
        target_text = identity_instruction_text(node.opcode, target_args, node.selected_values)
        diagnostics: tuple[LoweringDiagnostic, ...] = ()
        if not same_game:
    ...
    def _plan_syntax(self, node: SyntaxStatement, routine: str) -> CapabilityDecision:
        ...
        if node.selected_values and not same_game:
            return self._unsupported(
                ...
                code="value_selection.unsupported",
                message=(
                    "syntax expression depends on runtime-selected source values that "
                    "cannot be projected independently"
                ),
            )
    ...
    def _emit(self, node: SemanticOperation, decision: CapabilityDecision) -> CapabilityDecision:
        if self.backend_emitter is None or decision.strategy is LoweringStrategy.UNSUPPORTED:
            return decision
        try:
