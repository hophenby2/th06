...
from .semantic_ir import (
    ...
    DEFAULT_DIALECT_DECODER,
    semantic_operation_to_backend_event,
    semantic_domain,
)
from ..dialects.game_profile import profile_for_game
from ..dialects.reference import opcode_info, opcode_reference, opcode_signature
from ..dialects.semantics import bullet_shape_semantic, bullet_transform_mode_semantic, generation_for_game
...
def target_opcode_for_op_key(op_key: str, target: str) -> int | None:
    ...
    inheritance or compatibility must be present in the target reference or
    an explicit semantic lowering policy.
    """
...
def semantic_operation(
    ...
    *,
    ...
    referenced_operation = op_key_for_opcode(game, opcode)
    profile_operation = profile.bullet_dialect.operation_for(opcode)
    dialect_operation = (
        profile_operation
        if profile_operation and profile.bullet_dialect.implicit_manager
        else referenced_operation
    )
    operation = profile_operation or canonical_operation_id(dialect_operation, profile.opcode_family)
    domain = semantic_domain(
        operation,
        domain_for(game, opcode, info.name if info else ""),
    )
    mnemonic = operation if profile_operation and profile.bullet_dialect.implicit_manager else info.name if info else ""
    signature = profile.bullet_dialect.signature_for(opcode) or (info.signature if info else "")
    ...
    annotations: dict[str, object] = {
        ...
        "source_form": {"opcode_family": profile.opcode_family},
    }
    transform_form = profile.transform_dialect.form_for_opcode(opcode)
    if transform_form is not None:
        annotations["source_form"] = {
            "opcode_family": profile.opcode_family,
            "transform_write": transform_form.write_kind,
            "transform_parameter_set": transform_form.parameter_set,
        }
    macro_mode = profile.bullet_dialect.macro_mode_for(opcode)
    if macro_mode:
        annotations["macro_mode"] = macro_mode
    operands = DEFAULT_DIALECT_DECODER.decode(
        operation,
        profile.opcode_family,
        args,
        signature,
        transform_form.operand_names if transform_form is not None else None,
        game,
    )
    transform_mode = ""
    if transform_form is not None and operation in {"bullet.transform.replace", "bullet.transform.append"}:
        mode_operand = next((operand for operand in operands if operand.name == "mode"), None)
        if mode_operand is not None:
            transform_mode = bullet_transform_mode_semantic(game, mode_operand.value.source_text)
            annotations["transform_mode"] = transform_mode
    operands = contextualize_transform_operands(
        operation,
        operands,
        profile.sentinels,
        transform_mode,
    ...
    if operation in {"bullet.macro.configure", "bullet.visual.set"}:
        shape_operand = next((operand for operand in operands if operand.name == "bullet_type"), None)
        if shape_operand is not None:
            annotations["bullet_shape"] = bullet_shape_semantic(
                game,
                shape_operand.value.source_text,
            )
    if operation in {
        "bullet.formation.speeds.by_rank",
        "bullet.formation.counts.by_rank",
    } and len(args) > 1:
        annotations["rank_sample_count"] = (len(args) - 1) // 2
    return SemanticOperation(
        node_id=NodeId.for_statement(routine, int(line or 0)),
        operation=operation,
        domain=domain,
        operands=operands,
        guard=DifficultyGuard.from_marker(difficulty),
        provenance=Provenance(
            game=game,
            routine=routine,
            span=SourceSpan(source=source, start_line=int(line or 0), end_line=int(line or 0)),
            opcode=opcode,
            mnemonic=mnemonic,
            signature=signature,
            raw=raw,
...
def op_event(
    ...
    *,
    ...
    event = semantic_operation_to_backend_event(operation)
    event.pop("canonical_operation", None)
    policy = op_lowering_policy(str(event["op_key"]), list(args))
    if policy:
