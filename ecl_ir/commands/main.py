...
from ..canonical.op_ir import semantic_operation
from ..canonical.semantic_ir import SemanticModule
from ..compat.backend import choose_difficulty, compile_bullet_emitter, compile_ir_op_event, compile_object, definition_emitter_state, first_difficulty_group, normalize_backend_event, normalize_difficulty, parse_int_literal, wrap_ranked_lines
from ..dialects.anm_catalog import choose_script, remap_set_script
from ..dialects.reference import validate_opcode_args
from ..dialects.semantics import bullet_shape_semantic, encode_bullet_shape, encode_spread_style, generation_for_game, lifted_raw_coverage_policy, opcode_map_for, remap_raw_arg_by_semantic, spread_semantic, unsupported_bullet_transform_mode_reason
from ..integrations.luastg.backend import emit_luastg_file
...
def write_decl_output(text: str, output: str | Path | None, codec: DeclTextCodec) -> None:
    output_bytes = codec.encode(text)
    if output:
...
def inferred_function_params(program) -> dict[str, str]:
    ...
    for signature in getattr(program, "routine_signatures", []):
        if params.get(signature.name) or not signature.params:
            continue
        if signature.name in params:
            params[signature.name] = signature.params
    return params


def decoded_instruction_operation(ins, source_game: str):
    ...
    return semantic_operation(
        source_game,
        int(opcode),
        [str(arg) for arg in getattr(ins, "args", [])],
        getattr(ins, "line_no", 0),
        getattr(ins, "difficulty", None),
        raw=getattr(ins, "raw", ""),
    )


def is_fire_instruction(ins, source_game: str) -> bool:
    operation = decoded_instruction_operation(ins, source_game)
    return operation is not None and operation.operation == "bullet.fire"


def should_preserve_dynamic_bullet_config(ins, source_game: str) -> bool:
    operation = decoded_instruction_operation(ins, source_game)
    return (
        operation is not None
        and operation.domain == "bullet"
        and getattr(ins, "opcode", None) in {502, 503, 504, 505, 506, 507, 508, 523, 524, 525, 600, 602, 603, 604, 605, 606, 607, 608, 609, 611, 612, 624, 625, 626, 627, 628}
    )
...
def should_cover_lifted_raw_instruction(ins, obj, source_game: str, target: str) -> bool:
    ...
    if policy == "all":
        return True
    if is_fire_instruction(ins, source_game):
        return False
    if policy == "contiguous_setup_prefix":
        return True
    return not should_preserve_dynamic_bullet_config(ins, source_game)
...
def lower_raw_instruction_event(opcode: int, args: list[object], text: str, source_game: str, target: str, difficulty_literals: object = None, ir_op: dict[str, object] | None = None, context: dict[str, object] | None = None) -> list[str]:
    if ir_op:
        ir_op = normalize_backend_event(ir_op)
...
def emit_timeline_event(event: dict[str, object], source_game: str = "unknown", target: str = "", bullet_state: BulletLoweringState | None = None, context: dict[str, object] | None = None) -> list[str]:
    ...
    if kind == "instruction":
        ...
        args = event.get("args", [])
        semantic_node = event.get("semantic_op") or event.get("ir_op")
        semantic_event = normalize_backend_event(semantic_node) if isinstance(semantic_node, dict) else None
        is_bullet_semantic = semantic_event is None or semantic_event.get("domain") == "bullet"
        if (
        ...
        create_lowered = lower_bullet_create_opcode(opcode_int, list(args), source_game, target, bullet_state) if is_bullet_semantic else None
        if create_lowered:
            return wrap_event_rank(create_lowered, event, target)
        if is_bullet_semantic and generation_for_game(source_game) == "th12" and generation_for_game(target) == "th13_plus" and opcode_int == 509 and len(args) >= 4:
        ...
        fire_lowered = lower_bullet_fire_opcode(opcode_int, list(args), source_game, target, bullet_state) if is_bullet_semantic else None
        if fire_lowered:
            return wrap_event_rank(fire_lowered, event, target)
        config_lowered = lower_bullet_config_opcode(opcode_int, list(args), source_game, target, event.get("difficulty_literals", []), bullet_state) if is_bullet_semantic else None
        if config_lowered:
        ...
        transform_lowered = lower_bullet_transform_opcode(opcode_int, list(args), source_game, target, event.get("difficulty_literals", []), bullet_state) if is_bullet_semantic else None
        ...
        misc_bullet_lowered = lower_th13_misc_bullet_opcode(opcode_int, list(args), source_game, target, event.get("difficulty_literals", []), bullet_state) if is_bullet_semantic else None
        ...
        diff_float_lowered = lower_th13_diff_float_opcode(opcode_int, list(args), source_game, target, event.get("difficulty_literals", [])) if is_bullet_semantic else None
        ...
        lowered = lower_raw_instruction_event(opcode_int, list(args), text, source_game, target, event.get("difficulty_literals", []), semantic_node, context)
        if lowered:
...
def cmd_transpile(args: argparse.Namespace) -> int:
    program, objects = load_objects(args.input)
    output = emit_transpile(program, objects, args.target)
    codec = DeclTextCodec.from_source_bytes(Path(args.input).read_bytes())
    write_decl_output(output, args.output, codec)
    return 0
